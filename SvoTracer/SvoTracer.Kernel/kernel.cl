...
{
    float cone = coneSize(fabs(fast_length((float3)(
        _data->Origin.x - uLongToFloat(_data->Location.x),
        _data->Origin.y - uLongToFloat(_data->Location.y),
        _data->Origin.z - uLongToFloat(_data->Location.z)))), _data);
...
BlockData average(uint address, global Block *blocks, float C, WorkingData *_data)
{
    //Average like heck            
    return blocks[address].Data;
}

void requestChild(uint address, uchar depth, global uint *childRequestId, global ChildRequest *childRequests, uint maxChildRequestId, ushort tick, uchar treeSize, ulong3 location)
{
...
WorkingData* setup(int2 coord, TraceInputData _input)
{
    ...
    float h = _input.FoV[0] * native_divide((native_divide((float)_input.ScreenSize[0], 2) - (float)coord.x), (float)_input.ScreenSize[0]);
    float v = _input.FoV[1] * native_divide((native_divide((float)_input.ScreenSize[1], 2) - (float)coord.y), (float)_input.ScreenSize[1]);
    ...
    float3 dir = (float3)(
        native_cos(yaw) * native_cos(pitch), 
...
void helpDereference(global Block *blocks, global Usage *usage, global uint *parentSize, global bool *parentResidency, global Parent *parents, global uint2 *dereferenceQueue, global int *dereferenceRemaining, 
        ...
        if(usage[address2.y >> 3].Tick == USHRT_MAX - 1)
        {
            ...
            while(!finished)
            {
                if(parents[parent].ParentAddress == address2.x)
                {
                    ...
                    //Last element in the list so previous element becomes the last element
            ...
            // If the parent removed from the list was the first and last then the block is no longer multi parent
            if(first && last)
            {
                parentResidency[parent] = false;
...
void dereference(global Block *blocks, global Usage *usage, global uint *parentSize, global bool *parentResidency, global Parent *parents, global uint2 *dereferenceQueue, global int *dereferenceRemaining, 
...
uint findAddress(global Block *blocks, global Usage *usage, global uint *childRequestId, global ChildRequest *childRequests, global uint *parentSize, global bool *parentResidency, global Parent *parents, 
    global uint2 *dereferenceQueue, global int *dereferenceRemaining, global int *semaphor, global ulong *addresses, UpdateInputData inputData, uint address, uint depth)
{
...
__kernel void prune(global ushort *bases, global Block *blocks, global Usage *usage, global uint *childRequestId, global ChildRequest *childRequests, global uint *parentSize, global bool *parentResidency, 
    global Parent *parents, global uint2 *dereferenceQueue, global int *dereferenceRemaining, global int *semaphor, global Pruning *pruning, global BlockData *pruningBlockData, global ulong *pruningAddresses, 
    UpdateInputData inputData)
{
    ...
    {
        address = findAddress(blocks, usage, childRequestId, childRequests, parentSize, parentResidency, parents, dereferenceQueue, dereferenceRemaining, semaphor, pruningAddresses, inputData, address, myPruning.Depth);
        if(address = UINT_MAX)
            return;
    }
    else
...
__kernel void graft(global Block *blocks, global Usage *usage, global uint *childRequestId, global ChildRequest *childRequests, global uint *parentSize, global bool *parentResidency, global Parent *parents, 
    global uint2 *dereferenceQueue, global int *dereferenceRemaining, global int *semaphor, global Grafting *grafting, global Block *graftingBlocks, global ulong *graftingAddresses, global uint *holdingAddresses, 
    global uint *addressPosition, UpdateInputData inputData)
{
    ...
    while(inputData.GraftSize < addressPosition[0])
    {
                ...
                (workingTick < inputData.Tick && workingTick < (inputData.Tick - offset)))))
        {
            ...
            dereference(blocks, usage, parentSize, parentResidency, parents, dereferenceQueue, dereferenceRemaining, semaphor, usage[myAddressPosition].Parent, inputData.Tick);
    ...
    {
        address = findAddress(blocks, usage, childRequestId, childRequests, parentSize, parentResidency, parents, dereferenceQueue, dereferenceRemaining, semaphor, graftingAddresses, inputData, 
            address, myGrafting.Depth);
        if(address = UINT_MAX)
            return;
        if(blocks[address].Child != UINT_MAX)
            dereference(blocks, usage, parentSize, parentResidency, parents, dereferenceQueue, dereferenceRemaining, semaphor, address, inputData.Tick);
...
__kernel void traceVoxel(global ushort *bases, global Block *blocks, global Usage *usage, global uint *childRequestId, global ChildRequest *childRequests, __write_only image2d_t outputImage, TraceInputData _input)
{
    uchar depth = 1;
    bool inside = true;
    uchar chunkPosition;
                        ...
                        while(depth > (_data->N + 1) && !leaving(_data))
                        {
                            ...
                            while(inside && !leaving(_data))
                            {
                                ...
                                {                   
                                    ...
                                    {
                                        depth = _data->N + 2;
                                        if(saveVoxelTrace(average(localAddress, blocks, C, _data), _data))
                                        {
